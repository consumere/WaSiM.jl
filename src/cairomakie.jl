...
macro ncrm()
    ncrem = "C:/Users/Public/Documents/Python_Scripts/julia/ncremover.jl"
    include(ncrem)
end
macro rasrm()
    remer = "C:/Users/Public/Documents/Python_Scripts/julia/raster_remover.jl"
    include(remer)
end
macro nco(s)
    nconly(s)
...
macro sdjl()
    fx = "C:/Users/Public/Documents/Python_Scripts/julia/sd2.jl"
    include(fx)
end
#@sdjl
macro sf(s)
...
function dfonly(x::Vector{Any})
    ...
    z = try
        filter(f -> !occursin(r"^wq|xml|nc|png|svg|jpg", f), x)
    catch
...
"""
```
x::Union{String,Regex,Raster};ti::String="Fliesszeitsummen [h]",
        hide=false,c=(:greys,0.7),kwargs..., kw...)
```
kw... passed to heatmap!
"""
function mkfzs(x::Union{String,Regex,Raster};
    ti::String="Fliesszeitsummen [h]",
    lonlat=false, hide=false,
    ...
    fig = GeoMakie.Figure(fontsize=15)
    axs = Makie.Axis(fig[1, 1],
        title=ti,
        xticks=xt,
        xticklabelrotation=45,
        ...
        xminorticks=IntervalsBetween(5),
        yminorticks=IntervalsBetween(5))
    p1 = GeoMakie.heatmap!(axs, z;
        colormap=c,
        kw...)
    Makie.contour!(axs, z; color=:black,
        linewidth=0.5)
    Colorbar(fig[1, 2], p1,
        width=20, ticksize=20,
        tickalign=1)
