...
#@__DIR__
#joinpath(homedir(),".julia")|>cd
#in case of import errors.
#joinpath(homedir(),".julia/registries/General/Registry.toml")|>edit
...
using GLM: FormulaTerm, lm, coef, term, predict
...
import GeoFormatTypes #.EPSG as EPSG
...
Plots.theme(:dao) #activate latex style typesetting.

export addname, all_values_equal, aplot, bardf, bardfm, bardfm!, bargroup, barp, barsum, baryr, baryrmean, baryrsum, build_soil_dictionary, byear, calculate_folder_size, calculate_spi, cb, cdinto, cdof, climateplot, climateplot_dfold, cmdf, cmplot, cnt, cntcolread, cntcols, cntcolv, colorfunction, colsums, condasize, corrbar, correlogram, cpinto, cs, ct, ctg, ctl, ctl2, ctl3, ctlook, ctsum, dd, ddense, dec, denselog, denseplot, df_rollmean, dfavg, dfavg!, dfbar, dfilter, dfl, dfl!, dfm, dfm!, dfmo, dfon, dfonly, dfp, dfp!, dfpall, dfr, dfread, dfrib, dfroute, dfsp, dfsplog, dftrend, dpr, dpr!, dprbig, dsbar, dtrange, du, dubplot, eeread, extract_duration_from_xml, extract_layers, f1_score, fdd, fdf, fillmissing, filterdf, filterplot, filterplot!, findalls, findctl, findindf, findlog, floq, fparse, fread, freaddf, fsize, fsoil, fsz, ftp, ftplin, ftsp, fz, generate_export_statement, get_folder_size, getdf, getf, getm, getmoduleorder, getnames, ggofbatch, ggofjl, ggofjl_nosvg, glob, globdf, globf, gofbatch, gofbatch_nosvg, grec, grep_KGE, grep_files, grep_in_files, grep_with_context, gwread, hd, heat, heatraw, hombr, homes, hometeo, homg, homreg, ht, hyd2, hydro, hydro_f, hydrograph, hydromon, hyeval, include, interpolate_missing, irfan, isroute, jdd, jldf, jldfnm, jlt, jsrast, jsread, juliasize, kernelplot, kge, kge1, kge2, kge_df, kge_df3, kge_fread, kge_read, kge_rec, kge_recdf, kgedf, kgegrep, kgegrepr, kgerec, kgeval, kgewrite, klog, lambda, lastcol, lat, latx, ldf, ldfpall, lf, lg, listdfs, ll, llf, loadalldfs, loaddf, loadso, looks_like_number, lplot, lplotf, lpro, ls, luheat, luplot, luscatter, lutab, luvars, mall, malldf, mbx, merge_vectors, moisture_plot_with_confidence, monc, monc_f, moncol, monmean, monp, monsum, mos, mvwasim2, mywd, nctodf, nctodfo, nqp, nread, nse, nse2, nse_rec, nsegrep, nsegrepr, nseval, nsevalraw, nsx, odfr, old_waread, old_waread2, op, oplat, ovio, pall, pe, penman_monteith, pers, pew, pfix, platform, plot_duration_graph, plot_grouped_metrics, plot_rolling_mean, plotf, polygonize_raster, pout, print_lines_between_patterns, print_sorted_sizes, process_file2, process_folders_and_subfolders, project, pw, pww, pxm, pxm!, qall, qall_num, qba, qbb, qgk, qpl, qplot, qqp, qrtr, rds, read_between, read_df, read_landuse_data2, read_log_file, read_mhm, read_soildata, read_soildata_2, read_soildata_4, read_soildata_raw, read_until_flag, read_wq, readall, readalloutput, readallras, readbetween, readdf, readf, readmeteo, readmhm, readmhm2, readroute, rec, regand, remove_leap_days, rename_columns, rename_columns!, rename_duplicates, renamer, reorder_df, rglob, rmeq, rmeq_rec, rmlat, rmm, rmopt, rmqout, rolling_mean, route_from_dir, routeg, routg, rowmeans, rowsums, rsq, rsqgrep, runwasim, sdf, seldf, selt, skipyr, so_read, src_path, stplot!, strans, subset_dataframe_by_mask, subsum, tblcb, tbx, tdiff, te, theplot, tline, tline!, toMain, tocb, tovec, vars, varsdf, vecdf, vef, vef2, vg, vg2, vgctl, vgjl, vgjlrec, vgrep, vgrepl, vgrfiles, vgrr, vibx, vio, vjl, wa, waba, waba2, wain, waread, waread2, wcl, wintree, wqlen, wqpand, wqplot, wqsum, wread, writedesc, writewa, wslp, wslpath, xflow, xflow2, xflow_o, xread, yearsum, yrmean, yrsum, yrx, zscore

# # DATAFRAME Operations
# export hd, dfread, dfrib, dfsp, dfsplog, dfp, dfp!, dfpall,
# yrsum, yrmean, dfm, dfm!, dfmo, dfl, dfl!, dfilter,
# monmean, monsum, lastcol, yrx

# # generate_export_statement(wa)

# # File Operations
# export fsoil, readall, readalloutput, readallras, read_between, read_log_file,
#     read_mhm, read_soildata, read_soildata_2, read_soildata_4,
#     read_until_flag, read_wq, waread,
#     #csize,
#     pout,
#     project,
#     read_landuse_data2, readmeteo, readroute 
#     # readras, readras2, readrasrec
#     # pydf, pydf_to_julia,  #no need to export
    
# # Hydrological Functions
# export hydro, hydro_f, hydromon, hyeval,
#     kge, kge1, kge2, kge_df, kge_df3, kge_fread, kge_read,
#     kge_rec, kgedf, kgerec, kgeval,
#     yearsum, hyd2, xflow, hydrograph,
#     lambda, mos

# # Statistical Functions
# export addname, all_values_equal,
#     barsum, baryr,
...
#     dfread, dfsp, dfsplog,
#     dprbig, dsbar, dtrange, dubplot, eeread,
#     extract_layers, f1_score, fdf, fillmissing, filterdf, findalls, findctl, fparse, fread, freaddf, generate_export_statement, getdf, getf, getnames,
...
#     kge_df3, kge_rec, klog, ldf,
#     ldfpall, lg, listdfs, loadalldfs, loaddf, loadso,
#     lplot, lplotf, lpro,
#     mall, malldf, mbx,
...
#     readmhm, rec, rename_columns, rename_columns!, rename_duplicates,
#     renamer, reorder_df, rmm, rmopt, rmqout, routeg, routg,
#     rowmeans, rowsums, runwasim, seldf, selt, skipyr, so_read, stplot!, 
#     strans
...
enhanced Reader.
Read the text file, preserve line 1 as header column \n
```waread(x::String;station=false,proj=false,pts_and_data=false,src=GeoFormatTypes.EPSG(25832),dst=GeoFormatTypes.EPSG(4326))```
"""
function waread(x::String; station=false, proj=false, pts_and_data=false, src=GeoFormatTypes.EPSG(25832), dst=GeoFormatTypes.EPSG(4326))
    ms = ["-9999", "lin", "log", "--"]
    if station
        ...
        if ez |> eltype != Float64
            ez = ez |> x->tryparse.(Float64,x)
            xc = xc |> x->tryparse.(Float64,x)
            yc = yc |> x->tryparse.(Float64,x)
            no = no |> x->tryparse.(Float64,x)
        end
...
function kge_read(path::AbstractString, ext::AbstractString; return_dataframe=false)
    ...
    for file in readdir(path)
        ...
        if isfile(file_path) && endswith(file, ext) && !occursin(r"xml|qgk|fzt|ftz|log|ini|wq|txt|yrly|nc|tif|jpg|png|svg", file)
            try
                ...
                push!(results, (kge_value=kge_value, file_path=file_path))
            catch e
    ...
    return return_dataframe ? results : nothing
end
...
https://doi.org/10.1016/j.jhydrol.2009.08.003
"""
function kge(x::Vector{Float64}, y::Vector{Float64}; verbose=false)
    simulated, observed = x, y
    r = cor(observed, simulated)
    α = std(simulated) / std(observed)
    β = mean(simulated) / mean(observed)
    kge = 1 - sqrt((r - 1)^2 + (α - 1)^2 + (β - 1)^2)
...
```
dfl!(regex::Union{Regex,String,DataFrame};kw...)
```
"""
function dfl!(regex::Union{Regex,String,DataFrame}; kw...)
    if regex isa DataFrame
        df = regex
    else
        df = readdf(regex)
    end
    ...
    if (any(x -> occursin("year", x), names(df)))
        s = Symbol.(filter(x -> !occursin(r"year|date", x), names(df)))
        @df df Plots.plot!(:year, cols(s), yaxis=:log; kw...)
        Plots.annotate!([(20, 5, text(ti, 12, :left, :top, :green))])
    else
        s = Symbol.(filter(x -> !occursin("date", x), names(df)))
        @df df Plots.plot!(:date, cols(s), yaxis=:log; kw...)
        Plots.annotate!([(20, 5, text(ti, 12, :left, :top, :green))])
    end
end
...
function aplot(df::DataFrame)
    ...
    s = Symbol.(filter(x -> !occursin(r"date|year|month|time", x), names(df)))
    
    ti = try
        o = DataFrames.metadata(df) |> collect
        "AndrewsPlot of " * basename(o[1][2])
    catch
...
function getf(x::AbstractString; cwd::String=pwd())
    m = []
    for (root, dirs, files) in walkdir(cwd)
        for file in files
            if isfile(file) && occursin(Regex(x), file)
                nm = joinpath(root, file)
                push!(m, (nm))
            end
        end
...
newer see wawrite
"""
function writewa(file::AbstractString, df::DataFrame; hourstep::Int64=24)
    dout = copy(df)
...
"""
freaddf(ext::AbstractString) recursively reads all files with extension ext
"""
...
"""
resample by date
`DataFrames.combine(DataFrames.groupby(df, :year), y .=> sum .=> y)`
"""
...
function yrsum(x::DataFrame)
    df = copy(x)
    if any(map(x -> contains(x, "year"), names(df)))
        @warn "year column already exists! removing..."
        df = select(df, Not(:year))
    end
    y = filter(x -> !occursin(r"date|year|month|time"i, x), names(df))
    df[!, :year] = year.(df[!, :date])
    df_yearsum = DataFrames.combine(DataFrames.groupby(df, :year), y .=> sum .=> y)
    return (df_yearsum)
end
...
function yrmean(x::String)
    ...
    df[!, :year] = year.(df[!, :date])
    df_yrmean = DataFrames.combine(DataFrames.groupby(df, :year), y .=> mean .=> y)
    return (df_yrmean)
end
...
function bardfm(x::String)
    ...
    df[!, :year] = year.(df[!, :date])
    meandf = DataFrames.combine(DataFrames.groupby(df, :year), y .=> mean .=> y)
...
function monmean(x::String)
    ...
    #s = map(y -> Symbol(y),y)
    df[!, :month] = month.(df[!, :date])
    dmean = DataFrames.combine(DataFrames.groupby(df, :month), y .=> mean .=> y)
    return (dmean)
end

"""
DataFrames.combine(DataFrames.groupby(df, :month), y .=> mean .=> y)
"""
...
can also be sum.
dfr(r"qbas")|>i->rmm(i;fun=sum)|>wa.dfp
"""
function rmm(x::DataFrame; fun=mean)
    ...
    aggdat = DataFrames.combine(groupby(df, [:year, :month]),
...
```
"""
function readmhm2(x::AbstractString)
    ms = ["-9999", "-9999.0", "-9999.0000000"]
    df::DataFrame = CSV.read(x, DataFrame,
        missingstring=ms,
        drop=(i, nm) -> i == 1,
        #dateformat=Dict(Day Mon Year)
        stripwhitespace=true,
        delim=" ",
        ignorerepeated=true,
        silencewarnings=true,
        normalizenames=true)
    df.date = Date.(string.(df.Day, "-", df.Mon, "-", df.Year), "dd-mm-yyyy")
    #map(col -> col .= replace(col, -9999.0 => missing), eachcol(df))
    # for i in axes(df)[2]
...
"""
reads qgko files and returns a vector of DataFrames
"""
...
function routeg(input_file::String, output_file::String)
    open(output_file, "w") do output
        ...
        for line in eachline(input_file)
            ...
            if line_num > 50 && startswith(line, "[routing_model")
                in_range = true
...
function te(x::String)
    ...
    for row in eachrow(dy)
        ...
        printstyled(rpad("Tdiff for 100 days in $year is $ref [mm]", 45), color=:magenta)
        if ref <= 0
            println("\n")
        elseif ref <= 5
...
"""
simulated, observed plot; kw passed to plot
```
ftplin(df::DataFrame;kw...)
rename!(ndf, [:Simulated,:Observed,:Date])
```
"""
function ftplin(df::DataFrame; kw...)
    if size(df)[2] != 3
        #throw(@warn "wrong number of columns - using dfp!")
        @warn "wrong number of columns - using dfp!"
        @warn "need :Simulated,:Observed,:Date !"
...
"""
function kge_fread(;return_dataframe=false)
    if return_dataframe
        kge_read(pwd(), "outjl";return_dataframe=true)
    else
        kge_read(pwd(), "outjl")
    end
end
...
"""
```baryrsum(df::Union{DataFrame, Regex}; leg=:outertopright, kw...)```
Creates a grouped bar plot of yearly sums from a DataFrame or data loaded from files matching a regex pattern.
If the input contains a date column, the function automatically performs yearly summation.

# Arguments
- ```df```: Either a DataFrame or a Regex pattern to match files for reading with waread
- ```leg```: Legend position (default: :outertopright)
- ```kw...```: Additional keyword arguments passed to groupedbar plot

# Returns
- A Plots.Plot object showing the grouped bar chart

# Features
- Automatic yearly summation for DataFrames with date columns
- Smart x-axis tick handling for long time series
- Automatic title generation from metadata or current directory
- Configurable legend position
"""
function baryrsum(df::Union{DataFrame, Regex, String}; leg=:outertopright, kw...)
    # Handle input data
    working_df = if df isa Regex
        try
            waread2(df)
        catch e
            error("Failed to read data using waread2: ", e)
        end
    else
        df
    end

    # Check for date columns and perform yearly summation if needed
    date_cols = map(x -> occursin(r"date"i, x) && !occursin(r"year"i, x), names(working_df))
    
    if any(date_cols)
        working_df = yrsum(working_df)
    end

    # Validate required columns
    if !(:year ∈ propertynames(working_df))
        error("DataFrame must contain a 'year' column")
    end

    #all.([x <: AbstractFloat for x in eltype.(eachcol(working_df[!, Not(:year)]))])
    #[x isa AbstractFloat for x in eltype.(eachcol(working_df[!, Not(:year)]))]

    # Get numeric columns for plotting (excluding year/date columns)
    series_cols = Symbol.(filter(x -> !occursin(r"year|date"i, x), names(working_df)))
    cv = [x <: AbstractFloat for x in eltype.(eachcol(working_df[!,series_cols]))]
    #cv = [x <: AbstractFloat for x in eltype.(eachcol(working_df[!, Not(:year)]))]
    #check if any column is not a float
    if !all(cv)
        error("Non-numeric columns found in DataFrame, cannot plot")
    end
    # if isempty(series_cols) && 
    #     error("No numeric columns found for plotting")
    # end

    # Generate title
    title = try
        metadata_name = DataFrames.metadata(working_df) |> only |> last |> basename
        "$(basename(pwd())) $metadata_name"
    catch
        @warn "No basename in metadata, using default title"
        "Series of $(basename(pwd()))"
    end

    # Create the plot
    @df working_df groupedbar(
        :year,
        cols(series_cols),
        legend=leg,
        group = working_df.year,
        #xticks=yrticks[2],
        xrotation=45,
        xlabel="",
        ylabel="[mm]",
...
#     if length(df.year) > 20
#         lentime = size(df, 1)
#         slice_dates = range(1, lentime, step=lentime ÷ 8)
#         yrticks = (slice_dates, df.year[slice_dates])
#     else
#         yrticks = df.year
#     end
...
#         # y = filter(x->!occursin("date",x),names(df))
#         # df[!, :year] = year.(df[!,:date])
#         # df = DataFrames.combine(DataFrames.groupby(df, :year),
#         #     y .=> sum .=> y)
#     end
...
#         yrticks = (slice_dates, df.year[slice_dates])
#         #hacky.
#         #yrticks = vcat(yrticks[2],last(df.year))
#     else
#         yrticks = df.year
#     end
...
function dsbar(ds::DataFrame)
    ...
    bar(ds.name, ds.KGE, xlabel="Name", ylabel="KGE", legend=false,
        ...
        annotations=(ds.name, ds.KGE, ann),
        bar_width=0.6)
end
...
function dpr(a::Regex, b::Regex)
    a = waread(a)[!, Cols(1, :date)]
    b = waread(b)[!, Cols(1, :date)]
...
macro pwrs_str(s)
    open(`powershell -noprofile`, "w", stdout) do io
        print(io, s)
    end
end
#pwrs""" which python """
...
"""
kge barplot, whith color threshold
```
ds = kge_df3()
kgeval(ds::DataFrame;threshold::Float64=0.41)
```
"""
function kgeval(ds::DataFrame; threshold::Float64=0.41, miny::Float64=-1.0, maxy::Float64=0.5)
    ...
    p1 = Plots.bar(ds.name, ds.KGE, xlabel="Name", ylabel="KGE", legend=false,
        ...
        xrotation=45,
        #fmt = :png,
        ylims=(extrema(ds.KGE) .+ [miny, maxy]),
        size=(800, 600),
        fillcolor=ifelse.(ds.KGE .> threshold, "cornflowerblue", "coral2"),
...
function mbx(df::DataFrame; fun=mean)
    ...
    p = @df df StatsPlots.boxplot(
        ...
        legend=false)
    xticks!(0.5:11.5, month_abbr)
    means = DataFrames.combine(DataFrames.groupby(df, :Month), ln[1] => fun)
...
"""
``` DataFrames.combine(df, names(df) .=> mean) ```
"""
...
function getmoduleorder(file1::AbstractString)
    # Read file contents
    content = read(file1, String)

    # Use regex to find module names in square brackets
    pattern = r"^\[(.*?)\]"m
    names_inside_brackets = [match.captures[1] for match in eachmatch(pattern, content) if match !== nothing]
    #names_inside_brackets = DataFrame.subset(names_inside_brackets, x -> !isempty(x))
    # Create DataFrame with extracted names
    return DataFrame(Line=names_inside_brackets)
end
...
```
pall(files::Vector{DataFrame}; toyr=true, leg=:outertopright; kwargs...)
```
"""
function pall(files::Vector{DataFrame}; toyr=true, leg=:outertopright, kwargs...)
    #files = dfs    
    bns = try
    ...
    if (any(x -> occursin("year", x), names(df)))
        ...
        s = Symbol.(filter(x -> !occursin(r"year|date", x), names(df)))
        p = @df df Plots.plot(:date, cols(s), legend=leg; kwargs...)
        #, title=ti
    end
    return p
end
...
reduces + merges by date
```
mall(left::DataFrame, right::DataFrame)
```
"""
...
"""
```
cpinto(src::Vector{String}, dst::AbstractString;force=false)
mkdir("route-bak")
...
"""
function tovec(x::DataFrame, col::Any;verbose=false)
    df = select(x, col)
    if verbose
        println(names(df))
    end
...
function read_soildata_2(filename::String)
    ...
    tck = broadcast(x -> if (startswith(x[1], "thickness"))
            x[3]
        end, bks)
...
function dfonly(x::Union{Vector{Any},Vector{String}})
    ...
    z = try
        filter(f -> !occursin(r"^wq|xml|nc|png|zip|7z|svg|jpg", f), x)
    catch
...
function corrbar(a::Regex, b::Regex)
    ...
    for i in 1:size(df_A, 2)
        Plots.annotate!(i, correlations[i],
            Plots.text(ann[i], 8, :center, :top, :black;
...
function corrbar(a::DataFrame, b::DataFrame)
    ...
    p0 = Plots.bar(1:size(df_A, 2), correlations,
        ...
        fontfamily="JuliaMono", #if installed, would break otherwise
        #fontfamily="CascadiaMono", #would break otherwise
        left_margin=10mm,
        bottom_margin=2mm)
...
```
dfmo(x::Union{Regex,String,DataFrame};leg = :topright, fun=monmean,mode=:line)
```
"""
...
with DataFrame input
"""
function bardf(x::DataFrame; leg=:topright, kw...)
    df = copy(x)
...
"""
adds a plot to the current plot
"""
function dfm!(x::Union{Regex,String,DataFrame};
    ann=false,
    log=false,
...
tline(df::DataFrame, dtcol::Symbol;lab=true, kwargs...)
lab sets the trendline equation as label
"""
function tline(df::DataFrame, dtcol::Symbol=:date; lab=true, kwargs...)
    # Get the date column and column names for trendlines
    date_data = df[!, dtcol]
    trendline_cols = setdiff(names(df), [string.(dtcol)])
    ...
    for y_col in trendline_cols
        ...
        # change_indices = findall(diff(Dates.year.(date_data)) .!= 0)

        # # Add the first index and the last index
        # indices = [1; change_indices; length(date_data)]

        # # Convert the years to strings
        # yr_str = string.(Dates.year.(date_data)[indices])

        # Set the x-ticks to the indices where the year changes, and label them with the corresponding years
        #xticks!(p, indices, yr_str; xrotation=45)

        Plots.plot!(p, date_data,
            trendline.(1:length(date_data)),
            label=trendline_equations,
            #xticks = (indices,yr_str),
            xlims=(minimum(date_data), 
            maximum(date_data)),
            linewidth=2, linestyle=:dash; kwargs...)

        #     lentime = size(date_data,1)
        # slice_dates = range(1, lentime, step=lentime ÷ 8)
        # yrs = Dates.year.(date_data)
        # yrticks = (slice_dates, yrs[slice_dates])
...
(df::DataFrame; dtcol=:date, lab=false)
"""
function tline!(df::DataFrame; dtcol=:date, lab=false, kwargs...)
    # Get the date column and column names for trendlines
    date_data = df[!, dtcol]
    trendline_cols = setdiff(names(df), [string.(dtcol)])
...
"""
convience function for collect(start:step:stop)
```
dtrange(start, stop, step)
dtrange(DateTime(2004),DateTime(2005),Second(3600))
...
``` kernelplot(df::Union{String,DataFrame}; ann=true) ```
"""
function kernelplot(df::Union{String,DataFrame}; ann=true)
    ...
    dsu = qba()
    M = parse.(Float64, dsu[!, Cols(2, 4)]) |> y -> subset(y, 1 => ByRow(>(0.0))) |> Matrix
...
``` corrbar(a::Vector{Float64}, b::Vector{Float64}) ```
example:
```	wa.corrbar(r"sb05",r"qgk") ```
"""
#function corrbar(a::Any, b::Any)
function corrbar(a::Vector{Union{Missing, Float64}}, b::Vector{Union{Missing, Float64}})
    df_A = collect(a)
    df_B = collect(b)
    #ti = "$a vs $b"
    ti = raw""
...
function cmplot(; temp::Union{Regex,String}=r"^so_temper", prec::Union{Regex,String}=r"^so_prec", col=1)
    ...
    Plots.plot!(twinx(), tempvec,
        ylabel="Temperatur [°C]",
        guidefontfamily="Computer Modern",
        tickfontfamily="Computer Modern",
        color=:coral2,
        ylims=(minimum(tempvec) - 1.0, maximum(tempvec) + 1.0),
        annotations=(temp.month .+ 0.125,
            tempvec .+ 0.1, ann2, :center),
        label=false,
        linestyle=:dashdot,
...
"""
function strans(fn::String; src=GeoFormatTypes.EPSG(25832), dst=GeoFormatTypes.EPSG(4326))
...
function stplot!(fn::Union{String,DataFrame})
    if isa(fn, DataFrame)
        ...
        for i in 1:length(xc)
            pt = ArchGDAL.createpoint([xc[i], yc[i]])
            pt = ArchGDAL.reproject(pt, GeoFormatTypes.EPSG(25832), ProjString("+proj=longlat +datum=WGS84 +no_defs"))
            push!(pts, pt)
        end
...
"""
inserts winstring from clipboard and changes to wslpath
"""
function pe()
    try
        inp = clipboard()
...
"""
inserts winstring from clipboard and changes backslashes to forward slashes
"""
function pew()
    try
        in = clipboard()
    ...
    if x isa String
        ...
        if endswith(x, "shp")
            ...
            out = ArchGDAL.reproject(geom,
                GeoFormatTypes.GeoFormatTypes.EPSG(25832),
                GeoFormatTypes.GeoFormatTypes.EPSG(4326))
...
function qrtr(pt::Union{String,DataFrame}; fun=sum, agg=quarterofyear)
    ...
    #s = map(y -> Symbol(y),y)
    df[!, :date] .= agg.(df[!, :date])
    df_agg = DataFrames.combine(DataFrames.groupby(df, :date),
        y .=> fun .=> y)
    return (df_agg)
end
...
kw passed to title
    ´´´
    heat(x::Union{String,Regex,DataFrame};ann=true,
        cm::Union{String,Symbol}="RdBu",alpha=0.6,kw...)

    wa.heat(mydf[!,1:10],cm="Grays")
    wa.heat(mym[!,6:14],cm=:greens,alpha=.4)
    ´´´
"""
function heat(x::Union{String,Regex,DataFrame}; ann=true,
    cm::Union{String,Symbol}="RdBu", alpha=0.6, kw...)
    if x isa String
        printstyled("reading $x\n", color=:light_red)
        df = waread2(x; silencewarnings=false)
    ...
    md = cor(
        Matrix(select(df,
            Not(Cols(r"date|year|month|day"))))
    ) .^ 2
    ...
    p = heatmap(md,
        #c=:balance,
        #c=:lightrainbow, grep(r"gr",wa.cs)
        #c=:grays,
        alpha=alpha,
        c=clr,
        linewidth=0.8,
...
function moncol(x::DataFrame; fun=mean)
    dmean = copy(x)
    ...
    dmean = DataFrames.combine(DataFrames.groupby(dmean, :month),
        s .=> (dmean -> (fun(dmean), 1.96 * std(dmean) / sqrt(length(dmean))))
        .=>
            s)
...
df::DataFrame; confidence_level=0.95
"""
function monc(df::DataFrame; confidence_level=0.95, fun=mean)
    # Make a copy of the input DataFrame
    dmean = copy(df)
    ...
    dmean = DataFrames.combine(DataFrames.groupby(dmean,
        :month)) do group
        result = DataFrame(month=group.month)
        for col in columns
            values = group[!, col]
            mean_value = fun(values)
...
dfrib(df;confidence_level=.999,col=1)
"""
...
function klog(df::Union{String,DataFrame}; lin=false, ann=true)
    ...
    #dsu = qbb()|>last #recursive
    dsu = qba()
    M = parse.(Float64, dsu[!, cols]) |> y -> subset(y, 1 => ByRow(>(0.0))) |> Matrix
    mns = names(dsu[!, cols])
    B = kde(M)
    Plots.plot(B, legend=false, xlabel=mns[1], ylabel=mns[2])
...
"""
df = byear(some_qoutjl_df)
adds annoated scatterplot of metrics
"""
function pxm!(df::DataFrame; col=:ve, ann=true, usethresold=true, threshold=-0.410, all=false, kw...)
    p1 = Plots.plot!(xrotation=35)
    year = df.year
    if all
        nam = try
...
function rmopt()
    ...
    files = filter(file ->
            !occursin(r"(wq_|pl$|sh$|fzt|ftz|log$|ini|otherdata|intern)", file), files)
...
"""
filles each column of a dataframe with a value
``` fillmissing(df::DataFrame,fillval=-9999) ```
"""
function fillmissing(df::DataFrame, fillval=-9999)
    for col in eachcol(df)
...
function hyeval(
    ...
    try
        ...
        elseif freq == "yearly" || freq == "Y" || freq == "yr" || freq == "year"
            ...
            #DataFrames.combine(DataFrames.groupby(df, year.(df.date)), y .=> mean .=> y);
            #DataFrames.combine(DataFrames.groupby(df, quarterofyear.(df.date)), y .=> mean .=> y);
        end
    catch e
...
"""
using GLM with reader. see also tline
```
function dftrend(x::Union{String,Regex,DataFrame},col::Any=1;dtcol::Symbol=:date)
```
"""
function dftrend(x::Union{String,Regex,DataFrame}, col::Any=1; dtcol::Symbol=:date)
    if isa(x, DataFrame)
    ...
    DN = try
        Dates.value.(df[!, dtcol] .- minimum(df[!, dtcol]))
    catch
        @info "using dn as Int from $dtcol..."
        df[!, dtcol] .- minimum(df[!, dtcol])
    end

    df.dn .= DN
    # select the columns to use for the model
    id = propertynames(df[!, Not(dtcol)])
    ...
    plt = Plots.plot(df[!, dtcol],
        Matrix(select(df, dat)) |> vec,
        label=string(dat),
        legend=:topright)
    #@df df plot(:date, cols(dat), label=string(dat), legend=:topright)
    Plots.plot!(df[!, dtcol], predict(model, df),
        label=trendline_equations,
...
dfs = lutab()
```
luplot(dfs::Vector{DataFrame};par::String="LAI",kwargs...)
```
"""
function luplot(dfs::Vector{DataFrame}; par::String="LAI", kwargs...)
...
another ts reader
"""
function wain(x::AbstractString)
    #dfd = CSV.read(x, DataFrame, header = 1) #, limit = 3)
    #df1 = dfd[.!contains.(dfd[:, 1], r"[A-z]|-"), :]
    ms = ["-9999", "-9999.0"]
    df1 = try
        CSV.File(x,
            #silencewarnings=true,
            ...
            limit=50_000,
            header=1) |>
        z -> z[.!contains.(z.MM, r"[A-z]|-")] |>
             DataFrame
        #z[.!contains.(z.YY, r"[A-z]|-")] |>DataFrame
    catch e
        @error "Error reading file $x \n $e"
    end
    df1.date = [Dates.Date(parse(Int, row.YY), parse(Int, row.MM), parse(Int, row.DD)) for row in eachrow(df1)]
    select!(df1, Not([:YY, :MM, :DD, :HH]))
    metadata!(df1, "filename", x, style=:note)
    return df1
end
...
function xflow(df, q=[0.9, 0.1];
    ...
    plot!(
        ...
        alpha=0.8,
        #fillalpha=0.2,
        seriestype=:scatter, markersize=1.95, #.95,
        markershape=:diamond,
        label=L"q_{max}")
    plot!(df.Q10,
        color=:black,
...
```
cmdf(temp::DataFrame,prec::DataFrame;col::Int=1)
```
"""
function cmdf(temp::DataFrame, prec::DataFrame; col::Int=1)
    ...
    Plots.plot!(twinx(), tempvec,
        ...
        #    maximum(tempvec) + 1.0),
        ylims=(-5, 20), annotations=(temp.month .+ 0.125,
            tempvec .+ 0.1, ann2, :center),
        label=false,
        linestyle=:dashdot,
        linewidth=2,
...
function mos(x::AbstractString; pt::String=".")
    m = readdir(pt, join=true)
    fn = Grep.grep(r"_output.txt$", m)
...
function floq()
    ...
    filter!(x->!occursin(r".nc|.txt|Layer|qinf|qi__",x),lst)
    if (length(lst) < 4)
    ...
    xm = mall(lst)
    #selt(xm,r"22")
    #rename according to lst basename
    # Original column names and lst
    names_xm = names(xm)
...
try
    ...
    #@error "rasterfuncs.jl not found! $e"
    @error "smth went wrong! $e"
end

# ##add themes
# begin
#     PlotThemes._themes[:tst] = PlotTheme(
#         palette = PlotThemes.expand_palette(colorant"white",
#             PlotThemes.dao_palette; cchoices = [100]),
#         colorgradient = cgrad(:Dark2).colors,
#         fontfamily="Computer Modern",
...
#         xmirror = :true,
#         tick_direction = :out,
#         legend_font = Plots.Font("sans-serif", 10, :hcenter,
#             :vcenter, 0.0, colorant"black")
#     )
#     #test it
#     #showtheme(:tst)
#     PlotThemes._themes[:cooper] = PlotTheme(linewidth = 2,
#                              markersize = 10,
#                              fontfamily = "JuliaMono",
#                              titlefontsize = 24,
#                              guidefontsize = 18,
