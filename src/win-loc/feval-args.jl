...
if length(ARGS) == 0
    println("
    usage farg <infile> x y z 
    need path for controlfile <infile> 
        <optional arg 2: moving xmls with mvwasim2 to current dir>...
        <optional arg 3: mv + cleanup via ncrm and rmeq>...
        <optional arg 4: mv from vers1007 + cleanup via ncrm and rmeq>..."
        )
    exit()
...
@time using StatsPlots,DataFrames,CSV,Dates,Plots,Plots.PlotMeasures,Statistics
...
##########funcs##########
begin
    function mvwasim2(;ta=pwd(),vers1007::Bool=false,kw...) 
        if vers1007
            pt="C:/Users/chs72fw/Documents/EFRE_GIS/Hydrologie/WaSiM-ETH/wasimvzo64_10.07.02"
        else
...
if (isfile(infile) && length(ARGS) == 1)
    try
        begin
            try
            ...
            for (cnt,i) in enumerate(outd)
                local outname=onames[cnt]*"-lin.svg"     #"-lin.png"
            ...
            if  length(odat)>0 
                ...
                odat = first(odat)
                run(`wsl -d Ubuntu-18.04 -e bash $subpt $odat`)
            end
...
if length(ARGS) == 2
    ...
    begin
        ...
        #cd("D:/Wasim/regio/out/vt/vt3-f1/")
        odat = grep("otherdata",readdir())
        ...
        @error "workchain failed! \n $e"
    end
end

if length(ARGS) == 3
    println("mv folder wasimvzo64_10.06.05 and cleanup...")
    mvwasim2()
    try
        begin
            try
...
if length(ARGS) == 4
    println("mv folder wasimvzo64_10.07.02 and cleanup...")
    mvwasim2(;vers1007=true)
    try
        begin
            try
                ca = grep("inpath_hydro",readlines(infile))[end]
