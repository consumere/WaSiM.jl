...
"""
    labelsize(txt::AbstractString, fontpt::Int, size, rgx, rgy; inflate=1.0)
...
"""
function labelsize(txt::AbstractString, fontpt::Int, size, rgx, rgy; inflate=1.0)
    wpx = size[1]
    hpx = size[2]
    ...
    let
        ...
        sz = Makie.boundingbox(t).widths      # width and height of text
    end
...
# Keywords
- `size`: figure size (width x height in pixels)
...
function create_mesh(rgx, rgy,
    ...
    Δw, Δh = labelsize(labels[nmax], fontpt, size, rgx, rgy; inflate=flabel)
...
"""
    viz22(x::AbstractVector, y::AbstractVector,
        labels::AbstractVector{<:AbstractString};
        size=(600, 600),
        flabel=1.0,
        fdist=1.0,
...
# Keywords
- `size`: figure size (default: 600 x 600 pixels)
...
- nothing
"""
function viz2(x::AbstractVector, y::AbstractVector,
             labels::AbstractVector{<:AbstractString};
             size=(600, 600),
             flabel=1.0,
             fdist=1.0,
    ...
    mesh = create_mesh(extrema(x), extrema(y), labels;
                       size=size, flabel=flabel, fontpt=fontpt)
