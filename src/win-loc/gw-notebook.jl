...
# ╔═╡ 6eb8277e-344e-11ee-3b22-8fea074d2a9b
using DataFrames, CSV, Statistics, Dates, StatsPlots, Distributions

# ╔═╡ 3f3a1040-c487-463d-b0ef-fba991d222fe
#load funcs
begin
	    function waread(x::String)
			...
			header=1, missingstring=ms, normalizenames=true, types=Float64)
	        df = dropmissing(df, 1)
...
# ╔═╡ 13540b95-a31e-4161-85b2-a1a938c6f275
begin
...
# ╔═╡ 89d9fcbe-4810-4c00-935f-add553d27461
begin
		y = "D:/Wasim/regio/out/rc200/x18/f0/gwstrcm.x18.2017"
		#y = raw"D:\Wasim\regio\out\rc200\x17\cl\gwstrcm.x17.2017"
		#y = "D:/Wasim/regio/out/rc200/x3/cl/gwstrcm.x3.2005"
		#y = raw"D:\Wasim\regio\out\rc200\x17\spin0\gwn_rcm.x17.2017"
		#y = raw"D:\Wasim\regio\out\rc200\x17\spin0\gwstrcm.x17.2017"
		#y = raw"D:\Wasim\regio\out\rc200\x15\loc3\gwstrcm.x15.2017"
		#y = raw"D:\Wasim\regio\out\rc200\x17\spin0\so_geodetic_heads.x17.2017"
	
		
		gs = waread(y)
		dropmissing!(gs)
		#filter!(x->year.(x.date>2010),gs)
end

# ╔═╡ fd777223-cb64-41a9-b283-101f66efec42
begin
	#gs = filter(x->year.(x.date)>2010,gs)
	cd(dirname(y))
	println(propertynames(gs))
end

# ╔═╡ 4b88689d-0522-459a-9d3c-f362045cb8c1
dfp(gs)

# ╔═╡ ea985232-1800-4d36-bf8c-856001c6bd17
begin
	#nd = mall(gw, gs[!,Cols("C22","date")])
	nd = mall(gw, gs[!,Cols(1,"date")])
end

# ╔═╡ b6054719-bcf3-4e94-aa86-141aeb3ed652
...
# ╔═╡ 04d04395-15ab-44ae-acb5-7fa646f0650b
begin
	dropmissing!(nd)
	qplot(nd)
end

# ╔═╡ 0978e2c1-6752-4128-a688-91c5f864f165
baryrsum(nd)

# ╔═╡ b314bf5a-06a9-47db-af56-ee1ad9427bca
df = filter(x->year.(x.date)>2010,nd)

# ╔═╡ ac8a7384-2f51-4d53-a22e-799f9e10fa88
dfp(df)

# ╔═╡ 64607016-f8d8-44dd-941b-e57c4abd98c0
begin
	cd(dirname(y))
	z = first(filter(x->occursin(r"qbas",x),readdir()))
	println(z)
	bdf = waread(z)
	baryrsum(bdf)
end

# ╔═╡ 05430000-f5ee-4d58-a62f-81839bbd9c83
dfp(bdf)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "4963923770ae0d4b6cf719fa46fbcd35ddf8d28e"
