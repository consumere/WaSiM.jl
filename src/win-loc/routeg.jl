...
# if Sys.iswindows() #<-moved to loop; see below
#     ctf = ctl()
# else
#     #ctf = towsl.(ctl())
#     ctf = ctl()
...
#fls = fls[Not(3)]|>unique
fls = fls|>unique
#map(x->isroute(x;match=r"\$time"),fls)
#map(x->vcat(println("-> ",basename(x)),isroute(x;match=r"\$time")),fls)
map(x->vcat(println("-> ",replace(x, "\\"=> "/")),isroute(x;match=r"\$time")),fls)
#map(x->vcat(println("-> $x"),isroute(x;match=r"\$time")),fls)

#works in winjulia only
#julia --startup-file=no --color=yes "C:\Users\Public\Documents\Python_Scripts\julia\win\routeg.jl"
#fl="C:\Users\Public\Documents\Python_Scripts\julia\win\routeg.jl"
#jl $fl
