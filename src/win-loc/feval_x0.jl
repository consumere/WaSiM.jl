...
#wrkdir = ARGS[1]
##--> TO DO
wrkdir = "D:/Wasim/regio/out/rc200/x0/"
#setup()
wrkdir|>cd
...
ctlfile=raw"D:\Wasim\regio\control\rcm200_x0.ctl"
infile = ctlfile
outfile = "route.txt"
...
#run(`cat route.txt`)
open(io->readlines(io), "route.txt")|>z->map(x -> replace(x, '\t' => ' '), z)
#open(io->read(io, String), "route.txt")|>x->split(x,'\n')|>y->filter(!isempty, y)|>z->map(x -> replace(x, '\t' => ' '), z)
...
filter(3 => x -> occursin(r"^w"i, x), df)|> v -> sort(v,2)

filter!(3 => (x->!occursin(r"Wues|Pleich",x)),df)
#size(df)
...
# Warning: merge is empty on   Wuerzburg-Pleichach-qoutjl ! ...
# Warning: merge is empty on   Wuestenzell-qoutjl ! ...
#using Pipe
#@pipe df|>filter(_)
...
hcat(qdf,df)|>f -> sort(f,3,rev=false)|>PrettyTables.pretty_table
"Wuest"|>ftp
"Gem"|>ftp
"Oberth"|>ftp
r"Oberth"|>dpr
...
baryrmean(sim)
ar = "meg"|>readf
#ar|>x->filter()|>baryrsum

ar|>r->r[!,Cols(1,3,5,10,20,35,r"date"i)]|>baryrsum
ar|> z->z[!,Cols(1,r"ober"i,5,2,r"wolf"i,40,35,r"date"i)]|>baryrsum
...
"D:/Wasim/streu/out/f1/"|>cd
waba()
mym = ctl()
mym|>only|> y->replace(y,r"file " => "",r".ctl.*" => ".ctl")

infile=mym|>only|> y->replace(y,r"file " => "",r".ctl.*" => ".ctl")
outfile = "route.txt"
routeg(infile, outfile)
run(`wsl -e perl -i -pe 's/ß/ss/g;s/[\/]/_/g;s/_/-/g;s/[,,]//g;s/\xc4/Ae/g;s/\xd6/Oe/g;s/\xdc/Ue/g;s/\xe4/ae/g;s/\xf6/oe/g;s/\xfc/ue/g;s/\xdf/ss/g' $outfile`)
#run(`cat route.txt`)
...
#specfile="D:/Wasim/Tanalys/DEM/Input_V2/meteo/specdis_kmu.txt"
#specfile="D:/Wasim/Tanalys/DEM/Input_V2/meteo/s200obs"
specfile="D:/Wasim/streu/control/st_spec" 
sim = r"qges"|>glob|>first|>readdf
rename!(sim,map(x->replace(x,
    r"_" => "B"),names(sim)))
obs = readf(specfile)
simcols = names(sim)
obscols = names(obs)
df = CSV.read(pt,DataFrame,header=false,skipto=9,delim="\t",footerskip=1,lazystrings=false)
rename!(df,1=>"sim",2=>"obs",3=>"name")
df.name=map(x->replace(x*"-qoutjl",r"# " => ""),df[:,3])
