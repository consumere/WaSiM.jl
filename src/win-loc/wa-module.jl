...
##for wsl julia v1.8... all func-win.jl 
# 5.7.23
...
module wa
    ...
    function homg()
        pt="/mnt/d/Wasim/Goldbach/revision/"
        cd(pt)
    ...
    function hombr()
        pt="/mnt/d/Wasim/Tanalys/DEM/brend_fab/out/m4/"
        cd(pt)
        println("you are here: ",pwd())
    end
    ...
    function vgjl(snippet::AbstractString)
        ...
        cd("/mnt/c/Users/Public/Documents/Python_Scripts/julia/win")
        files = filter(file -> endswith(file, ".jl"), readdir())
        for file in files
            open(file) do f
                counter = 0 # Zähler initialisieren
    ...
    function vgjlrec(snippet::AbstractString)
        ...
        recursive grep
        """
        owd="/mnt/c/Users/Public/Documents/Python_Scripts/julia"
        for (root, dirs, files) in walkdir(owd)
            for file in files 
                if (endswith(file, ".jl"))
    ...
    function vgr(snippet::AbstractString)
        owd="/mnt/d/Fernerkundungsdaten/Klassifikation/R-Sessions"
    ...
    function vgctl(snippet::AbstractString)
        ...
        nwd4="/mnt/d/Wasim/regio/control/"
    ...
    #df=dfr(r"qgk")
    #"/mnt/d/Wasim/regio/out/rc200/v5/station4/"|>cd
    # df = readdf(r"qg")
    # #df = dfr(r"qg")
    # df = readdf(r"wind")
    # df = dfr(r"ou")
    ...
    function gofbatch()
        ...
        for i in arr
            x = basename(i)
            run(`cmd.exe /c Rscript "/mnt/d/Fernerkundungsdaten/Klassifikation/R-Sessions/gof3.R" $x`)
        end
    end
    ...
    function rpr(x::String)
        ...
        """
        #x="/mnt/d/temp/saale/output/thulba/v2/All_HydrologicResponseUnits.nc"
    ...
    function ggofbatch()
        ...
        for i in arr
            x = basename(i)
            run(`cmd.exe /c Rscript "/mnt/d/Fernerkundungsdaten/Klassifikation/R-Sessions/gof4.R" $x`)
        end
    end
    ...
    function ssup()
        include("/mnt/c/Users/Public/Documents/Python_Scripts/julia/win/smallfuncs.jl")
    end
    ...
    function ggofjl()
        ...
        """
        println("batch R Script for GOF")
            run(`cmd.exe /c Rscript "/mnt/d/Fernerkundungsdaten/Klassifikation/R-Sessions/gof5.R"`)
    end
    ...
    function waba()
        wpth="/mnt/c/Users/Public/Documents/Python_Scripts/julia/water-balance.jl"
        include(wpth)
        #img=load("waba-jl.png")
    ...
    function rmeq()
        ...
        files = filter(file -> 
        !occursin(r"pl|sh|csv|html|xml|fzt|ftz|log|ini|wq|yrly|nc|png|svg|txt", file)
        , readdir())
    ...
    macro sdjl() fx="/mnt/c/Users/Public/Documents/Python_Scripts/julia/sd2.jl";include(fx);end
    #@sdjl
    macro sf(s) glob(s);end

    macro listdir() ls();end
    #function listdir() fdi();end
    ...
    function sdd()
        lk="/mnt/c/Users/Public/Documents/Python_Scripts/julia/sd.jl"
        include(lk)
    end
    
    function wslp(winpt::AbstractString)
        """
        str=raw"C:\Users\Public\Documents\Python_Scripts\julia\frecurs.jl"
        str=raw"C:\Users\Public\Documents\Python_Scripts\julia\sd.jl"
        lk=wslp(str)
        include(lk)
    ...
    function qplot(df::DataFrame)
        ...
        df |> x -> qqplot(x[!,1],x[!,2], qqline = :fit, color = :grays) # erstellt ein QQ-Diagramm
        xr2 = round(cor(df[!,1], df[!,2])^2, digits=3)
        xr = maximum(df[!,1])
        annotate!(xr-0.5, 2.0, text("R² = $xr2", 12))
    end
