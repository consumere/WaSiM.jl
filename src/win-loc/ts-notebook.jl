...
# ╔═╡ 0210cc52-61ba-4510-8fe8-95043f0c6053
using DataFrames, CSV, Dates, PlotlyJS

# ╔═╡ 765f5b23-d24f-4f28-be3e-c58ce51abdda
using PlotlyBase 

# ╔═╡ b1f7479a-d1aa-49f6-8072-1ca384d03466
# plotly() 
#For saving to png with the `Plotly` backend `PlotlyBase` and `PlotlyKaleido` need to be installed.

# ╔═╡ 0f9c4e56-3c38-424c-a2cf-b094c6e03c71
file="/mnt/d/Wasim/regio/out/v8/vaporcm.v8.2000"

# ╔═╡ a5dba57d-2827-4c03-b727-30d6d63aaa2e
if endswith(file,".nc")
	println("need <timeseries file>...")
	exit()
end

# ╔═╡ 48d5836f-1adc-418b-9180-86850d1c5d6f
begin
	pt = splitdir(file)
	cd(pt[1])
	m = match(r".*[.]",basename(file))
	#outfile = contains(basename(file),".") ? string(m.match,"png") : 
...
# ╔═╡ 7ba719ed-9177-406f-a3a7-bbe9889314b7
function pline(path::AbstractString)
    ms=["-999","-9999","lin","log","LIN","LOG"]
    df = CSV.read(path,DataFrame,
    missingstring=ms,     #missingstring="-9999", #also windows
    delim="\t",comment="-",
...
# ╔═╡ b23e5462-c12f-4227-9bb3-e5d7678e06b7
...
# ╔═╡ 75bbc216-07b6-480f-9e41-62c36451d602
 #pline(file)

# ╔═╡ 3a694fbc-8ef0-4529-857b-b43e4a6fbd40
println("saving plotly plot to ",pwd(),"/",outfile,"...")

# ╔═╡ dc920146-a3a5-47da-817e-a67fe2251c62
savefig(wrapall(file),outfile)

# ╔═╡ 9910e3b3-b7e7-40b0-b753-9c4b4f8e7f27
println("done! ...")

# ╔═╡ ef025c2d-9c56-4887-b457-832da81a866e
function waread(path)
    ms=["-999","-9999","lin","log","LIN","LOG"]
    df = CSV.read(path,DataFrame,
    missingstring=ms,
    delim="\t",comment="-",
...
# ╔═╡ 9fce4286-d435-413b-96e3-ced5dcddca74
dd = waread(file)

# ╔═╡ 00a09a0c-d1d6-40c2-9219-4fea2cad7fff
z=plot(histogram2dcontour(x=dd[:,2],y=dd[:,end-1]))

# ╔═╡ 83c33c55-3967-4c72-b087-07d036b2b17f
savefig(z,"tst.png")

# ╔═╡ de3f4316-e3fe-45b4-870c-a324162ee1aa
pwd()
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "6334806c018e8f4c4a8aea5da6acc4f027bc9309"
